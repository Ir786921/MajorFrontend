...
function clickPrevious(){
  ...
  const targetTime = new Date().getTime() + 24/24 * (60*15)/60 * 60 * 1000;
...
async function getapi() {
    const response = await fetch(
      `https://timeworkapi-production.up.railway.app/api/aptitude/timework`
    );
    const result = await response.json();
    setApidata(result);
  }
    ...
    <div className="container-fluid  p-0">
      <div className="row">
        ...
        />
              ...
              <label htmlFor={`question_${path + index}`} id={index+1}  className="answer1 tw-text-xl tw-items-center tw-flex tw-justify-center" >
                {item}
        ...
        <div className="col-md-3">
           <div className=" tw-flex tw-justify-end">
            <button className=" tw-bg-green-500 tw-px-6 tw-py-[5px] tw-border-0 tw-text-xl tw-shadow-lg tw-rounded-md">End Test</button>
           </div>
           <hr />
           ...
           <div className=" tw-flex tw-gap-5 tw-bg-sky-100 tw-p-2">
            ...
            <div className=" tw-flex tw-justify-center tw-items-center"> <h3>
              User Name
            </h3></div>
           </div>
           
           <div className=" tw-flex tw-justify-start tw-gap-6 tw-bg-gray-100 tw-p-2">
                <div><h4 className=" tw-font-semibold tw-p-2">Time Left &nbsp; :</h4></div>
                <div className="  tw-flex tw-justify-center tw-items-center tw-p-2 tw-bg-gray-200 tw-rounded-md">
                  <h4 className=" tw-text-center tw-m-auto"> <CountdownTimer targetTime = {targetTime} /></h4>
                </div>
           </div>
           <hr />
           <br />
